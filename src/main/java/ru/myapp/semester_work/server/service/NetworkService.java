...
public class NetworkService {
    ...
    public void sendMessage(SocketChannel clientChannel, String clientInfo, String message) {
        if (clientChannel == null || !clientChannel.isOpen()) {
            LOGGER.error("Не удалось отправить сообщение. Канал клиента закрыт или он не существует.");
            return;
        }
        ...
        try {
            ...
            LOGGER.info("Отправлено сообщение {} : {}", clientInfo, message);
        } catch (IOException e) {
            LOGGER.error("Не удалось отправить сообщение клиенту {}", clientInfo, e);
        }
    }
}
